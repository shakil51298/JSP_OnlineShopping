...
<body>
	...
	try {
		...
		"select * from user_info inner join cart on cart.email='" + email + "' and cart.status='bill'");
		while (rs2.next()) {
	%>
	<h3>
		YOUR SHOP <i class="fa fa-shopping-bag" style="font-size: 30px"
			aria-hidden="true"></i> BILL
	</h3>
	<hr>
	<div class="left-div">
		<h3>
	...
	<table id="customers">
		...
		<%
		ResultSet rs1 = st
				.executeQuery("select * from cart inner join products where cart.p_id = products.id and cart.email = '" + email
	...
	<a href="ContinueShopping.jsp"><button class="button left-button">Continue
			Shopping</button></a>
	<a onclick="window.print();"><button class="button right-button">Print
			/ Save pdf</button></a>
	<br>
	<br>
	<br>
